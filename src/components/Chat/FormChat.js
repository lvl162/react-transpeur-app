...
function FormChat(props) {
  ...
  const { senderId, recipientId, name, arr, idChat, checkOther, header } =
    props;
  ...
  useEffect(() => {
    ...
    if (!checkOther) {
      ...
      dispatch(FetchChat2({ id: senderId, header: header }));
    }
    scrollToBottom();
  }, [idChat]);
  ...
  const elm =
    message.length > 0 ? (
      message.map((item, index) => {
        return (
          ...
          >
            ...
            <div>
              <span className='py-2 px-4 w-auto bg-blue-400 text-white rounded-full'>
                {item.content}
              </span>
            </div>
          </div>
      ...
      <h3 className='text-center text-4xl mt-40'>{'Say Hi 👋'}</h3>
    );
  ...
  return (
    <form className='w-full h-full flex flex-col justify-between'>
      ...
      <div className='flex items-center bg-white px-4 py-4 w-full'>
        ...
        />
        <button
          className='p-2 rounded-lg bg-blue-500 text-white flex items-center justify-center'
          onClick={sendMessage}
        >
