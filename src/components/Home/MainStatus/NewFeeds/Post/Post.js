...
const timeFrom = (date) => {
  ...
  if (interval > 7) return false; // expired
  return true;
};
...
function Post({ listPosts }) {
  ...
  const rolesLength = useSelector(
    (state) => state.CheckLogin.current.roles.length
  );

  // const ListPost = useSelector((state) => state.FetchListPost);
  const [list, setList] = useState(listPosts.slice(0, LIMIT));
  const showQuote = (num) => {
    if (num === option) {
      setOption(-1);
    } else {
  ...
  const deletePost = (item) => {
    ...
    swal({
      ...
      if (willDelete) {
        axios
          .get(`${API_URL}/api/travel/deactive/${item.id}`, {
            headers: {
  ...
  const ShowListPost = (list) => {
    let result;
    if (list.length > 0) {
      result = list
        .filter((item) => timeFrom(Date.parse(item.createAt)))
        .map((item, index) => {
          ...
          return (
            <div className='relative relative' key={index}>
              {option === index && (
                <div
                  className='hover:opacity-80 rounded-md absolute cursor-pointer right-0 shadow py-2 px-4'
                  style={{
                    zIndex: '999',
                    top: '-25px',
                    right: ' 2px',
                ...
                >
                  <div className='h-full w-full quote shadow-2xl flex items-center justify-center text-gray-600'>
                    Xóa Bài
                  </div>
                </div>
              )}
                ...
                <div className='border-b'>
                  <div className='flex items-center relative justify-between'>
                    <div className='flex items-center py-3 relative'>
                      <div
                        className={`avatar h-12 w-12 bg-cover rounded-full mr-6 cursor-pointer ${
                          item.creatorId === id
                            ? 'bg-avataImage'
                            : 'bg-avataImage2'
                        }`}
                      ...
                      <div>
                        <p className='font-bold'>
                          {`${item.infoUser.lastName} ${item.infoUser.firstName} `}
                          <span className='font-normal'>
                            <span>{`(${item.creatorUname})`}</span> đã thêm lịch
                            trình mới - Phương tiện :{' '}
                          </span>
                          {item.transport !== null ? item.transport : 'Xe máy'}
                        </p>
                    ...
                    {rolesLength > 1 && (
                      <div className=''>
                        <div className='p-2 duration-300 cursor-pointer rounded-full hover:bg-gray-100 flex items-center justify-center'>
                          <i
                            className='opacity-50 fas fa-ellipsis-h'
                            onClick={() => showQuote(index)}
                          ></i>
                        </div>
                      </div>
                    )}
                    {index + 1 === check && id !== item.creatorId && (
                      <div
                        className='duration-500 shadow-md p-4 bg-white absolute rounded-md'
                        style={{
                          left: '-130px',
                          top: '-160px',
                          width: '300px',
                      ...
                      >
                        <div className='flex items-center '>
                          <Link
                            to={`/profile/${item.creatorUname}`}
                            className='avatar bg-avataImage h-20 w-20 bg-cover rounded-full mr-6 cursor-pointer'
                          ></Link>
                          <div>
                        ...
                        <div className='flex justify-end items-center mt-2'>
                          <Link
                            to={`/chat/${item.creatorId}`}
                            className='rounded-md font-medium text-white bg-blue-500 py-2 px-10 flex items-center justify-center'
                          >
                    ...
                    {index + 1 === check && id === item.creatorId && (
                      ...
                      >
                        <div className='flex items-center '>
                          <div className='avatar bg-avataImage h-20 w-20 bg-cover rounded-full mr-6 cursor-pointer'></div>
                          <div>
                            <p className='font-bold py-3 text-xl'>{`${item.infoUser.lastName} ${item.infoUser.firstName}`}</p>
                        ...
                        <div className='flex justify-end items-center mt-2 font-medium opacity-80'>
                          <Link
                            className='p-2 bg-gray-200 rounded-md flex items-center justify-center'
                            to='/edit'
                          >
  ...
  const handleLoadMore = () => {
    if (list.length + LIMIT <= listPosts.length) {
      setList(list.concat(listPosts.slice(list.length, list.length + LIMIT)));
    } else {
      setList(list.concat(listPosts.slice(list.length)));
      setEnd(true);
    }
  };
  return (
    <div>
      <div className='mt-4'>{ShowListPost(list)}</div>
      {!end ? (
        <button
          className='bg-red-400 p-3 rounded-lg mb-3'
